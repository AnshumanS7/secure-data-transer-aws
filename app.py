...
UPLOAD_FOLDER = 'src\web-application\media\text-files'
UPLOAD_KEY = 'src\web-application\media\public-keys'
...
def download_public_key(username):
	for root,dirs,files in os.walk('src\web-application\media\public-keys'):
		for file in files:
			list = file.split('-')
			if list[0] == username:
...
def downloads_pk():
	username = []
	if(os.path.isfile("src\web-application\media\database\database_1.pickle")):
		pickleObj = open("src\web-application\media\database\database_1.pickle","rb")
...
def register_user():
	...
	if(os.path.isfile("src\web-application\media\database\database.pickle")):
		pickleObj = open("src\web-application\media\database\database.pickle","rb")
	...
	#Save/update pickle
	pickleObj = open("src\web-application\media\database\database.pickle","wb")
	pickle.dump(privatekeylist,pickleObj)
	pickleObj.close()
	pickleObj = open("src\web-application\media\database\database_1.pickle","wb")
